...
public class DrawerLayoutContainer extends FrameLayout {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if (ev != null && ev.getX() > AndroidUtilities.dp(150) && !drawerOpened && drawerPosition == 0)
            return false; //hossein
            ...
            if (allowOpenDrawer && parentActionBarLayout.fragmentsStack.size() == 1) {
                if (ev != null && (ev.getAction() == MotionEvent.ACTION_DOWN || ev.getAction() == MotionEvent.ACTION_MOVE) && !startedTracking && !maybeStartTracking) {
                    ...
                    if (startedTracking || drawerPosition != 0 && drawerPosition != drawerLayout.getMeasuredWidth()) {
                        float velX = velocityTracker.getXVelocity();
                        float velY = velocityTracker.getYVelocity();
                        boolean backAnimation = drawerPosition < drawerLayout.getMeasuredWidth() / 2.0f && (velX < AndroidUtilities.dp(120) || Math.abs(velX) < Math.abs(velY)) || velX < 0 && Math.abs(velX) >= AndroidUtilities.dp(120);
                        if (!backAnimation) {
                            openDrawer(!drawerOpened && Math.abs(velX) >= AndroidUtilities.dp(120));
                        } else {
                            closeDrawer(drawerOpened && Math.abs(velX) >= AndroidUtilities.dp(120));
                        }

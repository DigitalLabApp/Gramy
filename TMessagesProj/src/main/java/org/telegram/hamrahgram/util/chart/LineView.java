package org.telegram.hamrahgram.util.chart;
...
public class LineView extends View {
    ...
    public void setDataList(ArrayList<ArrayList<Integer>> dataLists, ArrayList<String> times) {
        this.times = times;
        ...
        if (dataLists != null && !dataLists.isEmpty()) {
            ...
            for (int k = 0; k < dataLists.size(); k++) {
                ...
                for (int i = 0; i < dataLists.get(k).size(); i++) {
                    ...
                    if (i > drawDotSize - 1) {
                        drawDotLists.get(k).add(new Dot(x, 0, x, y, dataLists.get(k).get(i), k, times.get(k)));
                    } else {
                        drawDotLists.get(k).set(i, drawDotLists.get(k).get(i).setTargetData(x, y, dataLists.get(k).get(i), k, times.get(i)));
                    }
                }
    ...
    protected void onDraw(Canvas canvas) {
            ...
            for (Dot d : drawDotLists.get(k)) {
                if (showPopupType == SHOW_POPUPS_All)
                    drawPopup(canvas, d.data, d.setupPoint(tmpPoint),
                            colorArray[k % colorArray.length], d.time);
        ...
        if (showPopup && selectedDot != null) {
            drawPopup(canvas,
                    selectedDot.data,
                    selectedDot.setupPoint(tmpPoint), colorArray[selectedDot.linenumber % colorArray.length], selectedDot.time);
        }
    }
     ...
     *               0 1 2 3 4 5
     */
    private void drawPopup(Canvas canvas, float num, Point point, int PopupColor, String time) {
        String numStr = time; //showFloatNumInPopup ? String.valueOf(num) : String.valueOf(Math.round(num));
...
//        mViewHeight = MeasureSpec.getSize(measureSpec);
    ...
    class Dot {
        ...
        Dot(int x, float y, int targetX, float targetY, float data, int linenumber, String time) {
            this.x = x;
            this.y = y;
            this.linenumber = linenumber;
            setTargetData(targetX, targetY, data, linenumber, time);
        }
        ...
        Dot setTargetData(int targetX, float targetY, float data, int linenumber, String time) {

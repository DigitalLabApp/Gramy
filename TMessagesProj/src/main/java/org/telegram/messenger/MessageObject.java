...
public class MessageObject {
    ...
    public MessageObject(TLRPC.Message message, AbstractMap<Integer, TLRPC.User> users, AbstractMap<Integer, TLRPC.Chat> chats, boolean generateLayout) {
        ...
        if (message.message != null) {
            if (blockMode && message.dialog_id == spamBotId) {
                if (message.message.contains("not possible")) {
                    Toast.makeText(ApplicationLoader.mContext, ApplicationLoader.mContext.getResources().getString(R.string.unblockcommandsent), Toast.LENGTH_LONG).show();
                    new Handler().postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            MessageObject.blockMode = false;
                            MessageObject.spamBotId = -1;
                        }
                    }, 3000);
                }
                if (message.message.contains("Good news, no limits are currently applied to your account. You’re free as a bird!") || message.message.contains("مژده، در حال حاضر اكانت شما هيچ محدودييتى ندارد. شما كاملاً آزاد هستيد.")) {
                    blockMessage = ApplicationLoader.mContext.getResources().getString(R.string.younotblock);
                    isBlock = false;
                } else if (message.message.contains("your account is now limited")) {
                    isBlock = true;
                    blockMessage = ApplicationLoader.mContext.getResources().getString(R.string.youareblock);
                }
                if (blockMessage.length() > 2)
                    Toast.makeText(ApplicationLoader.applicationContext, blockMessage, Toast.LENGTH_LONG).show();
                if (isBlock) {
                    SendMessagesHelper.getInstance().sendMessage("I was wrong, please release me", spamBotId, null, null, false, null, null, null);
                    if (!isBlock) {
                        new Handler().postDelayed(new Runnable() {
                            @Override
                            public void run() {
        ...
        //Hossein
            ...
            textPaint.setTypeface(AndroidUtilities.getTypeface(ApplicationLoader.applicationTypeFace));
        //-------------------
        ...
        if (messageText instanceof Spannable) {
            ...
            for (int a = 0; a < count; a++) {
                ...
                if (entity instanceof TLRPC.TL_messageEntityBold) {
                    spannable.setSpan(new TypefaceSpan(AndroidUtilities.getTypeface(ApplicationLoader.applicationTypeFace)), entity.offset, entity.offset + entity.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);

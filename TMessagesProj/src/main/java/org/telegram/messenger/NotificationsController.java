...
public class NotificationsController {
    ...
    protected void forceShowPopupForReply() {
        notificationsQueue.postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                if (!popupArray.isEmpty() && !AndroidUtilities.needShowPasscode(false) && !Database.getInstance().checkLock(pushMessages.get(0).getDialogId(), ApplicationLoader.applicationContext)) {
    ...
    public void processNewMessages(final ArrayList<MessageObject> messageObjects, final boolean isLast) {
        ...
        notificationsQueue.postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                if (!popupArray.isEmpty() && oldCount != popupArray.size() && !AndroidUtilities.needShowPasscode(false) && !Database.getInstance().checkLock(pushMessages.get(0).getDialogId(), ApplicationLoader.applicationContext)) {
    ...
    private String getStringForMessage(MessageObject messageObject, boolean shortMessage) {
        ...
        String msg = null;
        if ((int) dialog_id == 0 || AndroidUtilities.needShowPasscode(false) || UserConfig.isWaitingForPasscodeEnter || Database.getInstance().checkLock(pushMessages.get(0).getDialogId(), ApplicationLoader.applicationContext)) {
    ...
    private void showOrUpdateNotification(boolean notifyAboutLast) {
        ...
        try {
            ...
            if ((int) dialog_id != 0) {
                ...
                if (AndroidUtilities.needShowPasscode(false) || UserConfig.isWaitingForPasscodeEnter ||   Database.getInstance().checkLock(pushMessages.get(0).getDialogId(), ApplicationLoader.applicationContext)) {
                    photoPath = null;
                } else {
                    if (pushDialogs.size() == 1) {
                        if (chat != null) {
            ...
            if ((int) dialog_id == 0 || pushDialogs.size() > 1 || AndroidUtilities.needShowPasscode(false) || UserConfig.isWaitingForPasscodeEnter || Database.getInstance().checkLock(pushMessages.get(0).getDialogId(), ApplicationLoader.applicationContext)) {
                ...
                if (!MediaController.getInstance().isRecordingAudio()) {
                    if (choosenSoundPath != null && !choosenSoundPath.equals("NoSound")) {
                        if (choosenSoundPath.equals(defaultPath)) {
                            //Hossein---------------------------------------------
                            if (Database.getInstance().checkForRington(String.valueOf(dialog_id), ApplicationLoader.mContext) && Database.getInstance().getRingtoneById(String.valueOf(dialog_id), ApplicationLoader.mContext).length() > 1) {
                                String ring = Database.getInstance().getRingtoneById(String.valueOf(dialog_id), ApplicationLoader.mContext);
                                if (ring.equals("rings/3d_silent-glass.mp3") || ring.equals("rings/4f_here-I-am.mp3") || ring.equals("rings/Cool Notification 20.mp3") || ring.equals("rings/iPhone Original SMS Sound0.mp3") || ring.equals("rings/didid.mp3") || ring.equals("rings/sounds-792-the-little-dwarf.mp3")) {
                                    AssetFileDescriptor descriptor = ApplicationLoader.mContext.getAssets().openFd(Database.getInstance().getRingtoneById(String.valueOf(dialog_id), ApplicationLoader.mContext));
                                    MediaPlayer mp;
                                    mp = new MediaPlayer();
                                    ...
                                    String filePath = Database.getInstance().getRingtoneById(String.valueOf(dialog_id), ApplicationLoader.mContext);
    ...
    private void showExtraNotifications(NotificationCompat.Builder notificationBuilder, boolean notifyAboutLast) {
        ...
        for (int b = 0; b < sortedDialogs.size(); b++) {
            ...
            if (!ChatObject.isChannel(chat) && !AndroidUtilities.needShowPasscode(false) && !UserConfig.isWaitingForPasscodeEnter&& !Database.getInstance().checkLock(pushMessages.get(0).getDialogId(), ApplicationLoader.applicationContext)) {

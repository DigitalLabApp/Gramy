...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    //Hossein ---------------------------
    public ArrayList<TLRPC.TL_dialog> dialogsjusgroups = new ArrayList<>();
    ...
    public ArrayList<TLRPC.TL_dialog> dialogsFavoriteContact = new ArrayList<>();
    public ArrayList<TLRPC.TL_dialog> dialogsBlockedUsers = new ArrayList<>();
    public ArrayList<TLRPC.TL_dialog> dialogsOnlineUsers = new ArrayList<>();
    ...
    public ArrayList<TLRPC.User> update_user = new ArrayList<>();
    ...
    public int maxGroupCount = 4; //Hossein
    ...
    public void cleanup() {
        ...
        dialogsjusgroups.clear();
        dialogsFavs.clear();
        dialogsunread.clear();
        dialogsBots.clear();
        dialogsChannels.clear();
        dialogsUsers.clear();
        dialogsOnlineUsers.clear();
        dialogsBlockedUsers.clear();
        dialogsFavoriteContact.clear();
        dialogshidden.clear();
        //---------------------------------
        ...
        if (first) {
            ...
            if (dialog != null) {
                ...
                if (onlyHistory == 0 || onlyHistory == 3) {
                    ...
                    dialogsOnlineUsers.remove(dialog);
                    dialogsBlockedUsers.remove(dialog);
                    dialogsFavoriteContact.remove(dialog);
                    dialogsFavs.remove(dialog);
                    dialogsunread.remove(dialog);
                    dialogshidden.remove(dialog);
                    dialogsmegagroups.remove(dialog);
                    dialogsjusgroups.remove(dialog);
                    //------------------------------------------
    ...
    public void loadMessages(final long dialog_id, final int count, final int max_id, boolean fromCache, int midDate, final int classGuid, final int load_type, final int last_message_id, final boolean isChannel, final int loadIndex, boolean onlyTags) {
        loadMessages(dialog_id, count, max_id, fromCache, midDate, classGuid, load_type, last_message_id, isChannel, loadIndex, 0, 0, 0, false, onlyTags);
    }
    ...
    //hossein
    public void loadMessages(final long dialog_id, final int count, final int max_id, boolean fromCache, int midDate, final int classGuid, final int load_type, final int last_message_id, final boolean isChannel, final int loadIndex, final int first_unread, final int unread_count, final int last_date, final boolean queryFromServer, boolean onlyTags) {
        FileLog.e("tmessages", "load messages in chat " + dialog_id + " count " + count + " max_id " + max_id + " cache " + fromCache + " mindate = " + midDate + " guid " + classGuid + " load_type " + load_type + " last_message_id " + last_message_id + " index " + loadIndex + " firstUnread " + first_unread + " underad count " + unread_count + " last_date " + last_date + " queryFromServer " + queryFromServer);
        int lower_part = (int) dialog_id;
        MessagesStorage.getInstance().getMessagesByTagName(dialog_id, count, max_id, midDate, classGuid, load_type, isChannel, loadIndex);
    }
                    ...
                    for (int a = 0; a < updatesOnMainThread.size(); a++) {
                        ...
                        if (update instanceof TLRPC.TL_updatePrivacy) {
                            ...
                            if (currentUser != null) {
                                ...
                                if (ApplicationLoader.specificContacts.contains(String.valueOf(currentUser.id)))
                                    if (currentUser.first_name != null) //By Hossein
                                        try {
                                            PushNotification(ApplicationLoader.mContext, currentUser.first_name + ApplicationLoader.mContext.getResources().getString(R.string.changecontactusername), currentUser);
                                        } catch (Exception e) {
                                        }
                                    else {
                                        try {
                                            PushNotification(ApplicationLoader.mContext, currentUser.username + ApplicationLoader.mContext.getResources().getString(R.string.changecontactusername), currentUser);
                                        } catch (Exception e) {
                                        }
                                    }
                            ...
                            if (currentUser != null) {
                                //Hossein --------------------------------------------
                                if (ApplicationLoader.specificContacts.contains(String.valueOf(currentUser.id))) {
                                    if (!Database.getInstance().checkForRepeatNewPhoto(currentUser.id + "", update.date + "", ApplicationLoader.mContext)) {
                                        try {
                                            Database.getInstance().addNewPhotoUpdate(ApplicationLoader.mContext, currentUser.first_name + ":" + DateUtil.getCurrentShamsidate(), currentUser.last_name, String.valueOf(currentUser.id), DateUtil.getCurrentShamsidate(), FileLoader.getInstance().getDirectory(FileLoader.MEDIA_DIR_CACHE) + "/" + update.photo.photo_small.volume_id + "_" + update.photo.photo_small.local_id + ".jpg", update.date + "");
                                        } catch (Exception e) {
                                        }
                                        if (currentUser.first_name != null && currentUser.photo.photo_small != null) {
                                            try {
                                                PushNotification(ApplicationLoader.mContext, currentUser.first_name + " " + ApplicationLoader.mContext.getResources().getString(R.string.changedselfpic), currentUser);
                                            } catch (Exception e) {
                                            }
                                        } else {
                                            try {
                                                if (currentUser.photo.photo_small != null)
                                                    PushNotification(ApplicationLoader.mContext, currentUser.username + " " + ApplicationLoader.mContext.getResources().getString(R.string.changedselfpic), currentUser);
                                            } catch (Exception e) {
                                            }
                                        }
    ...
    public void sortDialogs(HashMap<Integer, TLRPC.Chat> chatsDict) {
        ...
        for (int a = 0; a < dialogs.size(); a++) {
            ...
            addDialog(d, getChat(-lower_id)); //Hossein
    ...
    private boolean isOnline(int id) {
        ...
        return user.id == UserConfig.getClientUserId() || user.status != null && user.status.expires > ConnectionsManager.getInstance().getCurrentTime() || MessagesController.getInstance().onlinePrivacy.containsKey(user.id);


    }

    //Hossein----------------------
    private void addDialog(TLRPC.TL_dialog d, TLRPC.Chat chat) {
        ...
        if (user != null) {
            if (user.bot) {
                ...
                dialogsUsers.add(d);
                if (ApplicationLoader.specificContacts.contains(d.id + "")) {
                    dialogsFavoriteContact.add(d);
                }
                if (blockedUsers.contains(((Long) d.id).intValue())) {
                    dialogsBlockedUsers.add(d);
                }
                if (isOnline(((Long) d.id).intValue())) {
                    dialogsOnlineUsers.add(d);

                }
            }
        }
        if (chat != null && chat.megagroup) dialogsmegagroups.add(d);
        if (chat != null && user == null && !chat.megagroup && !chat.broadcast)
            dialogsjusgroups.add(d);

        if (chat != null && chat.broadcast) dialogsChannels.add(d);
        if (ApplicationLoader.favoriteDialogs != null && ApplicationLoader.favoriteDialogs.contains(String.valueOf(d.id)))
            dialogsFavs.add(d);
        if (d.unread_count > 0) dialogsunread.add(d);

    }

    public int getMaxUserUnread() {
        int max = 0;
        int id = 0;
        for (TLRPC.TL_dialog dialog : dialogsUsers) {
            if (dialog.unread_count > max) {
                max = dialog.unread_count;
                id = (int) dialog.id;
            }
    ...
    public void openByUserNameasHidden(String username, final Activity activityContext) {
        final TLRPC.User user = getInstance().getUser(username);
        TLRPC.TL_contacts_resolveUsername req = new TLRPC.TL_contacts_resolveUsername();
        req.username = username;
        if (user == null) {
            final int reqId = ConnectionsManager.getInstance().sendRequest(req, new RequestDelegate() {
                        ...
                        public void run() {
                            if (error == null) {
                                ...
                                if (!res.chats.isEmpty()) {
                                    ...
                                    MessageObject.spamBotId = res.users.get(0).id;
                                    MessageObject.context = activityContext;
                                    SendMessagesHelper.getInstance().sendMessage("/start", res.users.get(0).id, null, null, false, null, null, null);
                                    //  SendMessagesHelper.getInstance().sendMessage("/start", res.users.get(0).id, null, null, false, false, null, null, null);
                                }
                            } else {
    ...
    public void PushNotification(Context ctx, String str, TLRPC.User current) {
        try {
            NotificationManager nm = (NotificationManager) ctx.getSystemService(Context.NOTIFICATION_SERVICE);
            Notification.Builder builder = new Notification.Builder(ctx);
            Intent notificationIntent = new Intent(ctx, LaunchActivity.class);
            notificationIntent.putExtra("favoritecontactid", current.id);
            ...
            builder.setContentTitle(ApplicationLoader.mContext.getResources().getString(R.string.khasprofileupdate));

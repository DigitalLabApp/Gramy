...
public class ApplicationLoader extends Application {
    ...
    public static String applicationTheme = "#ffffff";
    public static ArrayList<String> favoriteDialogs = new ArrayList<>();
    public static ArrayList<String> hiddenDialogs = new ArrayList<>();
    public static ArrayList<String> specificContacts = new ArrayList<>();
    public static String applicationTypeFace = "";
...
//Hossein-----------------------------------------------------
        ...
        textSize = 16;
        if (Database.getInstance().readSetting("2", mContext).equals("0"))
            applicationTypeFace = "fonts/is.ttf";
        else applicationTypeFace = Database.getInstance().readSetting("2", mContext);
        if (!Database.getInstance().readSetting("3", mContext).equals("0")) {
            if (Database.getInstance().readSetting("3", mContext).contains("#")) {
                applicationTheme = Database.getInstance().readSetting("3", mContext);
            } else {
                if (Database.getInstance().readSetting("3", mContext).contains("#")) {
                    applicationTheme = Database.getInstance().readSetting("3", mContext);
                } else {
                    int int_value = Integer.parseInt(Database.getInstance().readSetting("3", mContext));
                    applicationTheme = "#" + Integer.toHexString(int_value).substring(2);
                }
            }
        } else
            applicationTheme = "#08b9c4";
        if (Database.getInstance().getHiddenDialogs(mContext) != null)
            hiddenDialogs = Database.getInstance().getHiddenDialogs(mContext);
        else
            hiddenDialogs.add("Test");
        if (Database.getInstance().getFavoriteDialogs(mContext) != null)
            favoriteDialogs = Database.getInstance().getFavoriteDialogs(mContext);
        else favoriteDialogs.add("Test");

        if (Database.getInstance().getUpdate(mContext) != null)
            specificContacts = Database.getInstance().getUpdate(mContext);
        else
            specificContacts.add("Test");
        //--------------------------------------------------------
    ...
    synchronized public Tracker getDefaultTracker() {
        if (mTracker == null) {
            ...
            mTracker = analytics.newTracker("UA-83654556-4");

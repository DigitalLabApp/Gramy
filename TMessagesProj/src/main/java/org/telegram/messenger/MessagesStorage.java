...
public class MessagesStorage {
    ...
    //hossein
    public void advancedClearCache(final long did, boolean photo, boolean video, boolean audio, boolean document, boolean music) {
        ...
        storageQueue.postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                File docPath = new File(telegramPath, "Telegram Documents");
                File forDelete = null;
                for (File file : filesToDelete) {
                    if (photo) {
                        forDelete = new File(imagePath.getPath(), file.getName());
                        if (forDelete.exists()) {
                            forDelete.delete();
                            file.delete();
                        }
                    ...
                    if (video) {
                        forDelete = new File(videoPath.getPath(), file.getName());
                        if (forDelete.exists()) {
                            forDelete.delete();
                            file.delete();
                        }
                    ...
                    if (audio) {
                        forDelete = new File(audioPath.getPath(), file.getName());
                        if (forDelete.exists()) {
                            forDelete.delete();
                            file.delete();
                        }
                    ...
                    if (document) {
                        if (!file.getPath().contains(".mp3")) {
                            forDelete = new File(docPath.getPath(), file.getName());
                            if (forDelete.exists()) {
                                forDelete.delete();
                                file.delete();
                            }
                    ...
                    if (music) {
                        if (file.getPath().contains(".mp3")) {
                            file.delete();
                            new File(docPath.getPath(), file.getName()).delete();
                            new File(audioPath.getPath(), file.getName()).delete();
                            new File(imagePath.getPath(), file.getName()).delete();
                            new File(videoPath.getPath(), file.getName()).delete();
                        }
                    }
                }
            }
    ...
    public void getMessagesByTagName(final long dialog_id, final int count, final int max_id, final int minDate, final int classGuid, final int load_type, final boolean isChannel, final int loadIndex) {
                storageQueue.postRunnable(new Runnable() {
                    @Override
                    public void run() {
                        ...
                        try {
                            ...
                            if (lower_id != 0) {
                                if (load_type != 1 && load_type != 3 && minDate == 0) {
                                    if (load_type == 2) {
                                        cursor = database.queryFinalized("SELECT inbox_max, unread_count, date FROM dialogs");
                                        if (cursor.next()) {
                                        ...
                                        cursor.dispose();
                                        if (!queryFromServer) {
                                            cursor = database.queryFinalized(String.format(Locale.US, "SELECT min(mid), max(date) FROM messages  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = messages.mid)", dialog_id)); //where1
                                            if (cursor.next()) {
                                            ...
                                            cursor.dispose();
                                            if (min_unread_id != 0) {
                                                cursor = database.queryFinalized(String.format(Locale.US, "SELECT COUNT(*) FROM messages  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = messages.mid)", dialog_id, min_unread_id)); //where2
                                                if (cursor.next()) {
                                            ...
                                            cursor = database.queryFinalized(String.format(Locale.US, "SELECT COUNT(*) FROM messages  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = messages.mid)", dialog_id)); //where3
                                            if (cursor.next()) {
                                                existingUnreadCount = cursor.intValue(0);
                                            }
                                            cursor.dispose();
                                            if (existingUnreadCount == count_unread) {
                                                cursor = database.queryFinalized(String.format(Locale.US, "SELECT min(mid) FROM messages  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = messages.mid)", dialog_id)); //where4
                                                if (cursor.next()) {
                                                    messageMaxId = max_id_query = min_unread_id = cursor.intValue(0);
                                                    if (messageMaxId != 0 && channelId != 0) {
                                                        messageMaxId |= ((long) channelId) << 32;
                                            ...
                                            cursor = database.queryFinalized(String.format(Locale.US, "SELECT start, end FROM messages_holes", dialog_id, max_id_query, max_id_query));
                                            boolean containMessage = !cursor.next();
                                            cursor.dispose();

                                            if (containMessage) {
                                                cursor = database.queryFinalized(String.format(Locale.US, "SELECT min(mid) FROM messages  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = messages.mid)", dialog_id, max_id_query)); //where5
                                                if (cursor.next()) {
                                ...
                                cursor = database.queryFinalized(String.format(Locale.US, "SELECT start FROM messages_holes", dialog_id)); // WHERE uid = %d AND start IN (0, 1)"
                                if (cursor.next()) {
                                    isEnd = cursor.intValue(0) == 1;
                                    cursor.dispose();
                                } else {
                                    cursor.dispose();
                                    cursor = database.queryFinalized(String.format(Locale.US, "SELECT min(mid) FROM messages  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = messages.mid)", dialog_id));//where6
                                    if (cursor.next()) {
                                ...
                                if (load_type == 3 || queryFromServer && load_type == 2) {
                                    cursor = database.queryFinalized(String.format(Locale.US, "SELECT max(mid) FROM messages  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = messages.mid)", dialog_id)); //where7
                                    if (cursor.next()) {
                                    ...
                                    if (containMessage) {
                                        cursor = database.queryFinalized(String.format(Locale.US, "SELECT start FROM messages_holes", dialog_id, max_id_query, max_id_query));
                                        if (cursor.next()) {
                                    ...
                                    if (containMessage) {
                                        ...
                                        cursor = database.queryFinalized(String.format(Locale.US, "SELECT start FROM messages_holes", dialog_id, max_id_query));
                                        if (cursor.next()) {
                                        ...
                                        cursor.dispose();
                                        cursor = database.queryFinalized(String.format(Locale.US, "SELECT end FROM messages_holes", dialog_id, max_id_query));
                                        if (cursor.next()) {
                                        ...
                                        if (holeMessageMaxId != 0 || holeMessageMinId != 1) {
                                            ...
                                            cursor = database.queryFinalized(String.format(Locale.US, "SELECT * FROM (SELECT m.read_state, m.data, m.send_state, m.mid, m.date, r.random_id, m.replydata, m.media, m.ttl FROM messages as m LEFT JOIN randoms as r ON r.mid = m.mid " + "SELECT * FROM (SELECT m.read_state, m.data, m.send_state, m.mid, m.date, r.random_id, m.replydata, m.media, m.ttl FROM messages as m LEFT JOIN randoms as r ON r.mid = m.mid", dialog_id, messageMaxId, holeMessageMinId, count_query / 2, dialog_id, messageMaxId, holeMessageMaxId, count_query / 2));
                                        } else {
                                            cursor = database.queryFinalized(String.format(Locale.US, "SELECT * FROM (SELECT m.read_state, m.data, m.send_state, m.mid, m.date, r.random_id, m.replydata, m.media, m.ttl FROM messages as m LEFT JOIN randoms as r ON r.mid = m.mid" + "SELECT * FROM (SELECT m.read_state, m.data, m.send_state, m.mid, m.date, r.random_id, m.replydata, m.media, m.ttl FROM messages as m LEFT JOIN randoms as r ON r.mid = m.mid", dialog_id, messageMaxId, count_query / 2, dialog_id, messageMaxId, count_query / 2));
                                        }
                                    } else {
                                        cursor = null;
                                    }
                                    ...
                                    cursor = database.queryFinalized(String.format(Locale.US, "SELECT start, end FROM messages_holes", dialog_id, max_id)); // WHERE uid = %d AND start >= %d AND start != 1 AND end != 1 ORDER BY start ASC LIMIT 1"
                                    if (cursor.next()) {
                                    ...
                                    cursor.dispose();
                                    if (holeMessageId != 0) {
                                        cursor = database.queryFinalized(String.format(Locale.US, "SELECT m.read_state, m.data, m.send_state, m.mid, m.date, r.random_id, m.replydata, m.media, m.ttl FROM messages as m LEFT JOIN randoms as r ON r.mid = m.mid  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = m.mid)", dialog_id, minDate, messageMaxId, holeMessageId, count_query)); // WHERE m.uid = %d AND m.date >= %d AND m.mid > %d AND m.mid <= %d ORDER BY m.date ASC, m.mid ASC LIMIT %d"
                                    } else {
                                        cursor = database.queryFinalized(String.format(Locale.US, "SELECT m.read_state, m.data, m.send_state, m.mid, m.date, r.random_id, m.replydata, m.media, m.ttl FROM messages as m LEFT JOIN randoms as r ON r.mid = m.mid  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = m.mid)", dialog_id, minDate, messageMaxId, count_query)); // WHERE m.uid = %d AND m.date >= %d AND m.mid > %d ORDER BY m.date ASC, m.mid ASC LIMIT %d"
                                    }
                                } else if (minDate != 0) {
                                    if (messageMaxId != 0) {
                                        long holeMessageId = 0;
                                        ...
                                        cursor.dispose();
                                        if (holeMessageId != 0) {
                                            cursor = database.queryFinalized(String.format(Locale.US, "SELECT m.read_state, m.data, m.send_state, m.mid, m.date, r.random_id, m.replydata, m.media, m.ttl FROM messages as m LEFT JOIN randoms as r ON r.mid = m.mid  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = m.mid)", dialog_id, minDate, messageMaxId, holeMessageId, count_query)); //WHERE m.uid = %d AND m.date <= %d AND m.mid < %d AND (m.mid >= %d OR m.mid < 0) ORDER BY m.date DESC, m.mid DESC LIMIT %d"
                                        } else {
                                            cursor = database.queryFinalized(String.format(Locale.US, "SELECT m.read_state, m.data, m.send_state, m.mid, m.date, r.random_id, m.replydata, m.media, m.ttl FROM messages as m LEFT JOIN randoms as r ON r.mid = m.mid  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = m.mid) ", dialog_id, minDate, messageMaxId, count_query)); //WHERE m.uid = %d AND m.date <= %d AND m.mid < %d ORDER BY m.date DESC, m.mid DESC LIMIT %d"
                                        }
                                    } else {
                                        cursor = database.queryFinalized(String.format(Locale.US, "SELECT m.read_state, m.data, m.send_state, m.mid, m.date, r.random_id, m.replydata, m.media, m.ttl FROM messages as m LEFT JOIN randoms as r ON r.mid = m.mid  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = m.mid)", dialog_id, minDate, offset_query, count_query)); // WHERE m.uid = %d AND m.date <= %d ORDER BY m.date DESC, m.mid DESC LIMIT %d,%d"
                                    }
                                } else {
                                    cursor = database.queryFinalized(String.format(Locale.US, "SELECT max(mid) FROM messages", dialog_id)); // WHERE uid = %d AND mid > 0"
                                    if (cursor.next()) {
                                        last_message_id = cursor.intValue(0);
                                    }
                                    cursor.dispose();
                                    ...
                                    long holeMessageId = 0;
                                    cursor = database.queryFinalized(String.format(Locale.US, "SELECT max(end) FROM messages_holes", dialog_id));// WHERE uid = %d"
                                    if (cursor.next()) {
                                        holeMessageId = cursor.intValue(0);
                                        if (channelId != 0) {
                                            holeMessageId |= ((long) channelId) << 32;
                                    ...
                                    cursor.dispose();
                                    if (holeMessageId != 0) {
                                        cursor = database.queryFinalized(String.format(Locale.US, "SELECT m.read_state, m.data, m.send_state, m.mid, m.date, r.random_id, m.replydata, m.media, m.ttl FROM messages as m LEFT JOIN randoms as r ON r.mid = m.mid  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = m.mid)", dialog_id, holeMessageId, offset_query, count_query)); // WHERE m.uid = %d AND (m.mid >= %d OR m.mid < 0) ORDER BY m.date DESC, m.mid DESC LIMIT %d,%d"
                                    } else {
                                        cursor = database.queryFinalized(String.format(Locale.US, "SELECT m.read_state, m.data, m.send_state, m.mid, m.date, r.random_id, m.replydata, m.media, m.ttl FROM messages as m LEFT JOIN randoms as r ON r.mid = m.mid  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = m.mid)", dialog_id, offset_query, count_query)); // WHERE m.uid = %d ORDER BY m.date DESC, m.mid DESC LIMIT %d,%d"
                                    }
                                }
                            } else {
                                isEnd = true;
                                if (load_type == 1) {
                                    cursor = database.queryFinalized(String.format(Locale.US, "SELECT m.read_state, m.data, m.send_state, m.mid, m.date, r.random_id, m.replydata, m.media, m.ttl FROM messages as m LEFT JOIN randoms as r ON r.mid = m.mid  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = m.mid)", dialog_id, max_id, count_query)); //WHERE m.uid = %d AND m.mid < %d ORDER BY m.mid DESC LIMIT %d"
                                } else if (minDate != 0) {
                                    if (max_id != 0) {
                                        cursor = database.queryFinalized(String.format(Locale.US, "SELECT m.read_state, m.data, m.send_state, m.mid, m.date, r.random_id, m.replydata, m.media, m.ttl FROM messages as m LEFT JOIN randoms as r ON r.mid = m.mid  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = m.mid)", dialog_id, max_id, count_query)); // WHERE m.uid = %d AND m.mid > %d ORDER BY m.mid ASC LIMIT %d"
                                    } else {
                                        cursor = database.queryFinalized(String.format(Locale.US, "SELECT m.read_state, m.data, m.send_state, m.mid, m.date, r.random_id, m.replydata, m.media, m.ttl FROM messages as m LEFT JOIN randoms as r ON r.mid = m.mid  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = m.mid)", dialog_id, minDate, offset_query, count_query)); // WHERE m.uid = %d AND m.date <= %d ORDER BY m.mid ASC LIMIT %d,%d"
                                    }
                                } else {
                                    if (load_type == 2) {
                                        cursor = database.queryFinalized(String.format(Locale.US, "SELECT min(mid) FROM messages  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = messages.mid)", dialog_id)); // WHERE uid = %d AND mid < 0"
                                        if (cursor.next()) {
                                            last_message_id = cursor.intValue(0);
                                        }
                                        cursor.dispose();

                                        cursor = database.queryFinalized(String.format(Locale.US, "SELECT max(mid), max(date) FROM messages  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = messages.mid)", dialog_id)); // WHERE uid = %d AND out = 0 AND read_state IN(0,2) AND mid < 0"
                                        if (cursor.next()) {
                                            min_unread_id = cursor.intValue(0);
                                            max_unread_date = cursor.intValue(1);
                                        }
                                        cursor.dispose();
                                        if (min_unread_id != 0) {
                                            cursor = database.queryFinalized(String.format(Locale.US, "SELECT COUNT(*) FROM messages  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = messages.mid)", dialog_id, min_unread_id)); // WHERE uid = %d AND mid <= %d AND out = 0 AND read_state IN(0,2)"
                                            if (cursor.next()) {
                                                count_unread = cursor.intValue(0);
                                            }
                                            cursor.dispose();
                                    ...
                                    cursor = database.queryFinalized(String.format(Locale.US, "SELECT m.read_state, m.data, m.send_state, m.mid, m.date, r.random_id, m.replydata, m.media, m.ttl FROM messages as m LEFT JOIN randoms as r ON r.mid = m.mid  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = m.mid)", dialog_id, offset_query, count_query)); // WHERE m.uid = %d ORDER BY m.mid ASC LIMIT %d,%d"
                                }
                            }
                            ...
                            if (!replyMessages.isEmpty()) {
                                if (!replyMessageOwners.isEmpty()) {
                                    cursor = database.queryFinalized(String.format(Locale.US, "SELECT data, mid, date FROM messages  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = messages.mid)"));// WHERE mid IN(%s)", TextUtils.join(",", replyMessages)));
                                } else {
                                    cursor = database.queryFinalized(String.format(Locale.US, "SELECT m.data, m.mid, m.date, r.random_id FROM randoms as r INNER JOIN messages as m ON r.mid = m.mid  WHERE EXISTS(SELECT * FROM tagscontent WHERE tagscontent.mid = m.mid)"));// WHERE r.random_id IN(%s)", TextUtils.join(",", replyMessages)));
                                }
                                while (cursor.next()) {
                                    NativeByteBuffer data = cursor.byteBufferValue(0);
                                    if (data != null) {
                            ...
                            for (TLRPC.Message m : res.messages)
                                Log.d("qweeweeeeee", "message:" + m.message + "--id:" + m.id);
        ...
        storageQueue.postRunnable(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    //444
                    database.executeFast("UPDATE dialogs SET unread_count = 0, unread_count_i = 0 WHERE did = " + did).stepThis().dispose();
                    database.executeFast("DELETE FROM messages WHERE uid = " + did + " AND media = 0").stepThis().dispose();
                    database.executeFast("DELETE FROM bot_keyboard WHERE uid = " + did).stepThis().dispose();
                    database.executeFast("DELETE FROM media_counts_v2 WHERE uid = " + did).stepThis().dispose();
                    database.executeFast("DELETE FROM media_v2 WHERE uid = " + did).stepThis().dispose();
                    database.executeFast("DELETE FROM messages_holes WHERE uid = " + did).stepThis().dispose();

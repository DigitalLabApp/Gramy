...
import org.telegram.hamrahgram.util.ActionbarAnimation;
...
public class ActionBar extends FrameLayout {
    ...
    private ImageView spider; //Hossein
    ...
    private boolean isDialogsActivity = false; //Hossein
    ...
    public void setDialogsActivity(boolean isDialogsActivity) {
        this.isDialogsActivity = isDialogsActivity;
        if (isDialogsActivity)
    ...
    private void createTitleTextView() {
        ...
        titleTextView.setTypeface(AndroidUtilities.getTypeface(ApplicationLoader.applicationTypeFace));
        //Hossein
    ...
    public void hideBackButton() {
        ...
        backButtonImageView.setImageResource(0);
        backButtonImageView.setVisibility(GONE);


    }

    public void addOnTitleAction(View.OnClickListener action) {
        this.titleTextView.setOnClickListener(action);
    }
    ...
    public void updateSpider() {
        ...
        if (spider == null) {
            spider = new ImageView(getContext());
            addView(spider, 0, LayoutHelper.createFrame(AndroidUtilities.dp(20), AndroidUtilities.dp(20), Gravity.LEFT));
            spider.setScaleType(ImageView.ScaleType.CENTER);
            ViewGroup.MarginLayoutParams marginLayoutParams = (ViewGroup.MarginLayoutParams) spider.getLayoutParams();
            marginLayoutParams.setMargins(AndroidUtilities.dp(50), (android.os.Build.VERSION.SDK_INT >= 21) ? (AndroidUtilities.statusBarHeight - AndroidUtilities.dp(5)) : 0, 0, 0);
            // else
            //    marginLayoutParams.setMargins(AndroidUtilities.dp(50), AndroidUtilities.dp(-10), 0, 0);
            spider.setVisibility(View.GONE);
        }
        if (database.readSetting("7", ApplicationLoader.mContext).equals("1")) {
            spider.setImageDrawable(ActionbarAnimation.getSpiderAnimation());
            ((AnimationDrawable) spider.getDrawable()).stop();
            ((AnimationDrawable) spider.getDrawable()).start();
            spider.setVisibility(View.VISIBLE);
        } else {
            spider.setVisibility(View.GONE);
            spider.setImageDrawable(null);
        }
        spider.bringToFront();
    }
